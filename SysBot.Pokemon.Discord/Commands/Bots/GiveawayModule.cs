...
namespace SysBot.Pokemon.Discord
{
    ...
    public class GiveawayModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        public async Task GiveawayUploadAsync([Summary("Upload Type: item or pokemon")] string poolType, [Summary("Display Name")] string name, [Summary("Tag")] string tag)
        {
            ...
            {
                poolName = SysCord<T>.ITEM_POOL;
            }
            else if (poolType == "pokemon")
            {
                poolName = SysCord<T>.POKEMON_POOL;
            }
            else
            {
            ...
            await AddTradeToQueueAsync(code, Context.User.Username, GetPKM(null), sig, PokeRoutineType.FlexTrade, PokeTradeType.GiveawayUpload, Context.User, entry).ConfigureAwait(false);
        }
        ...
        public async Task DisplayGiveawayPoolCountAsync()
        {
            ...
            var activePool = poolDB.GetPool(SysCord<T>.POKEMON_POOL);
        ...
        {
            var poolDB = Info.Hub.GiveawayPoolDatabase;
            var activePool = poolDB.GetPool(SysCord<T>.ITEM_POOL);
            List<string> lines = new();
        ...
        public async Task SearchPokemonPoolAsync([Remainder] string search)
        {
            ...
            var poolDB = Info.Hub.GiveawayPoolDatabase;
            var activePool = poolDB.SearchPool(SysCord<T>.POKEMON_POOL, searchString);
            List<string> lines = new();
        ...
        public async Task SearchItemPoolAsync([Remainder] string search)
        {
            var searchString = "%" + search + "%";
            var poolDB = Info.Hub.GiveawayPoolDatabase;
            var activePool = poolDB.SearchPool(SysCord<T>.ITEM_POOL, searchString);
            List<string> lines = new();
        ...
        public async Task GiveawayAsync([Summary("ID/Name of Pokemon to Recieve")] string Item)
        {
            ...
            {
                entry = poolDB.GetEntry(SysCord<T>.POKEMON_POOL, poolId);
                if (entry == null)
                {
                    await ReplyAsync($"\"{content}\" is not a valid Pool ID, use \"{Info.Hub.Config.Discord.CommandPrefix}pokemonpool\" for a full list of available Pokemon and provide the entries ID.").ConfigureAwait(false);
                    return;
            ...
            await AddTradeToQueueAsync(code, Context.User.Username, GetPKM(entry.PK8), sig,  PokeRoutineType.LinkTrade, PokeTradeType.Giveaway, Context.User, entry).ConfigureAwait(false);
        }
        ...
        public async Task GiveawayItemAsync([Summary("ID/Name of Item to Recieve")] string Item)
        {
            ...
            {
                entry = poolDB.GetEntry(SysCord<T>.ITEM_POOL, poolId);
                if (entry == null)
                {
                    await ReplyAsync($"\"{content}\" is not a valid Pool ID, use \"{Info.Hub.Config.Discord.CommandPrefix}itempool\" for a full list of available Items and provide the entries ID.").ConfigureAwait(false);
                    return;
        ...
        public async Task GiveawayEntryDelete([Summary("Type (item/pokemon)")] string pooltype, [Summary("Pool Entry ID")] string id)
        {
            ...
            if (entry != null)
            {
                LogUtil.LogInfo($"Deleting {entry.Name} from {poolName}", nameof(GiveawayModule<T>));
        ...
        private async Task AddTradeToQueueAsync(int code, string trainerName, T pk, RequestSignificance sig, PokeRoutineType routine, PokeTradeType type, SocketUser usr, GiveawayPoolEntry poolEntry)
        {
            if (!pk.CanBeTraded())
            {
            ...
            await QueueHelper<T>.AddToQueueAsync(Context, code, trainerName, sig, pk, PokeRoutineType.LinkTrade, PokeTradeType.Specific, poolEntry).ConfigureAwait(false);
        }
        ...
        private async Task ListUtil(string nameMsg, string entry)
        {
            ...
            if (entry.Length > 1024)
            {
                ...
                {
                    var splice = TradeExtensions<T>.SpliceAtWord(entry, index, 1024);
                    index += splice.Count;
                    if (splice.Count == 0)
                        break;
